...
> ⚡ **ทางลัด:** ไม่ต้องไล่รันทีละบล็อกแล้ว — เปิดไฟล์ **`supabase-setup.sql`** (อยู่ข้างไฟล์นี้)
> วางทั้งไฟล์ใน SQL Editor แล้ว Run ครั้งเดียว ได้ครบทั้ง 12 ตาราง + นโยบาย RLS ทั้งหมด (รันซ้ำได้ปลอดภัย)
...
## 🎲 หอคอยท้าทายรายสัปดาห์ (tower_rush)
รัน `supabase-setup.sql` เวอร์ชันล่าสุดอีกครั้ง (มีตาราง `tower_rush` เพิ่ม) — ถ้าไม่มีตาราง เกมยังเล่นโหมดนี้ได้ แต่จะไม่มีกระดานอันดับ
